...
import { Customer, UpdateCustomerRequest, CustomerStatus, CustomerTier, Industry } from '../../types/customer';
...
interface EditCustomerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: number, data: UpdateCustomerRequest) => Promise<void>;
...
export const EditCustomerModal: React.FC<EditCustomerModalProps> = ({
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        assignedAccountManagerId: customer.assignedAccountManagerId,
        assignedAccountManagerName: customer.assignedAccountManagerName || '',
      });
      setError(null);
    }
  }, [customer]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to update customer account.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <ModalPortal>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-scale-up max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/50">
          <div>
            <h3 className="text-lg font-bold text-slate-100">Edit Customer: {customer.name}</h3>
            <p className="text-xs text-slate-400 mt-0.5">Modify account parameters, contract value, and account manager</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5 flex items-center justify-between">
                ...
                {isEmployee && customer?.assignedAccountManagerId && (
                  <span className="text-[10px] text-amber-400/90 font-normal flex items-center gap-1">
                    <Lock className="w-3 h-3" /> Assigned (Admin/Manager Managed)
                  </span>
                )}
                {isEmployee && !customer?.assignedAccountManagerId && (
              ...
              <select
                ...
                disabled={Boolean(isEmployee && customer?.assignedAccountManagerId)}
                className={`w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-sm focus:outline-none focus:border-indigo-500 ${
                  Boolean(isEmployee && customer?.assignedAccountManagerId)
                    ? 'opacity-70 cursor-not-allowed bg-slate-950/80'
                    : ''
                }`}
              >
                ...
                {users.map((u) => (
                  <option key={u.id} value={u.id}>
                    {u.name} ({u.roleName}) {isEmployee && u.id === user?.id ? '(You)' : ''}
                  </option>
                ))}
              </select>
            </div>
