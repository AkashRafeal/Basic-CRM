...
import { CreateLeadRequest, LeadStatus, LeadSource } from '../../types/lead';
...
export const CreateLeadModal: React.FC<CreateLeadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.firstName.trim() || !formData.email.trim()) {
      setError('Please provide first name and a valid email address.');
      return;
    }
    ...
    <ModalPortal>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-scale-up max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/50">
          <div>
            <h3 className="text-lg font-bold text-slate-100">Create New Sales Lead</h3>
            <p className="text-xs text-slate-400 mt-0.5">Add a new prospect to your CRM sales pipeline</p>
          </div>
          <button
            onClick={onClose}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Daily Planet Media"
                  className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-sm focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea
                ...
                placeholder="Details about customer requirements, budget, timeline, key stakeholders..."
                className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-sm focus:outline-none focus:border-indigo-500"
              />
            </div>
          </div>
