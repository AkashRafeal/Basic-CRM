...
import { CreateDealRequest, DealStage, DealType, DealPriority, PipelineStageConfig, DealItem } from '../../types/deal';
...
export const CreateDealModal: React.FC<CreateDealModalProps> = ({
  ...
  const [formData, setFormData] = useState<CreateDealRequest>({
    ...
    amount: 0,
    probability: 10,
    expectedCloseDate: new Date(Date.now() + 86400000 * 30).toISOString().split('T')[0],
  ...
  useEffect(() => {
    if (isOpen) {
      const initialProb = stageConfigs?.find(c => c.stage === 'QUALIFICATION')?.probability ?? 10;
      setFormData({
        dealName: '',
        stage: 'QUALIFICATION',
        amount: 0,
        ...
        customerId: undefined,
        customerName: '',
        assignedToUserId: isEmployee && user ? user.id : undefined,
        assignedToUserName: isEmployee && user ? user.name : '',
        description: '',
        items: [],
      });
      setDealItems([]);
  ...
  }, [isOpen, user, isEmployee, stageConfigs]);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-scale-up max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/50">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-100">Create New Deal / Opportunity</h3>
              <p className="text-xs text-slate-400 mt-0.5">Register high-value sales pipeline opportunities and forecasting</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-4 flex-1">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Wayne Enterprises Cloud Infrastructure Migration"
          ...
          <div>
            ...
            {isEmployee ? (
              <div className="relative">
                ...
                <input
                  type="text"
                  value={`${user?.name} (Assigned to self)`}
                  disabled
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea
                ...
                placeholder="Scope of work, deliverables, competitive risks, discount terms..."
                className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-slate-200 text-sm focus:outline-none focus:border-indigo-500"
              />
            </div>
          </div>
